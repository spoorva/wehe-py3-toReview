...
class ClientObj(object):
    ...
    def __init__(self, incomingTime, realID, id, ip, replayName, testID, historyCount, extraString, connection,
		...
		#/data/Rec../Repla../2020/month/day/tracerouteResults
        self.tracerouteFolder = self.targetFolder + 'tracerouteResults/'
    ...
    def setTraceRoute(self, name):
        self.tracerouteDump = name
        self.trc_rt = traceroute(outputFile=name, targetFolder=self.tracerouteFolder, id=self.id)
...
class SideChannel(object):
    ...
    def handle(self, connection, address):
        ...
        if good2go:
            ...
            dClient.setTraceRoute('_'.join(['server', realID, replayName, extraString, historyCount, testID]))
            try:
        ...
        #resultsFolder = Configs().get('resultsFolder') + '/' + realID + '/'
    ...
    def side_channel_callback(self, *args):
		...
		#Starting traceroute for the client
        #Command = "paris-traceroute -n " + str(dClient.id)
        #print(subprocess.check_call(Command, shell=True, env={'LD_LIBRARY_PATH':'/usr/local/lib'}))

        tracerouteRes = dClient.trc_rt.start()
        print("in replay server")
        #print(tracerouteRes)
