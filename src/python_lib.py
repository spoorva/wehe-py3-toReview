...
#class and methods to call traceroute functionality on client object
#called in side_channel_callback after tcpdump stops
#Usage : ClientObj.setTraceroute._
class traceroute(object) :

    def __init__(self, outputFile=None, targetFolder='./', interface=None, id=None):
        ...
        self.outputFile = 'traceroute_' + time.strftime('%Y-%b-%d-%H-%M-%S', time.gmtime()) + '.json'
        #else:
        #    self.outputFile = 'traceroute_' + outputFile + '.json' 
        self.outputFile = targetFolder + self.outputFile
		
    def create_json_from_traceroute(self, inputFile):
        ...
        for lines in all_lines :
            ...
            if (lines != "") :
                ...
                if ((len(words)) == 5):
                    ...
                    if "." in words[1]:
                         v4ExceptLast = words[1].rsplit('.', 1)[0]
                         anonymized_ip = v4ExceptLast + '.0'
                    elif ":" in words[1]:
                         v6ExceptLast = words[1].rsplit(':', 1)[0]
                         anonymized_ip = v6ExceptLast + ':0000'
                    else:
                         anonymized_ip = words[1]
        ...
        with open(self.outputFile, 'w') as writeFile:
            json.dump( dump_list, writeFile)
        
    def start(self):
		
		#change "Command" if you need to change the parameters for traceroute cmd
        Command = "paris-traceroute -n " + str(self.id)
        
        tmpFile = 'temp.txt'
        output = open(tmpFile, 'w')
            #subprocess.check_call(Command, shell=True, stdout=output,  env={'LD_LIBRARY_PATH':'/usr/local/lib'})
            #subprocess.Popen(Command, shell=True, stdout=output,  env={'LD_LIBRARY_PATH':'/usr/local/lib'})
        #self.create_json_from_traceroute(tmpFile)
        process = subprocess.Popen(Command, shell=True, stdout=output, stderr=subprocess.PIPE, env={'LD_LIBRARY_PATH':'/usr/local/lib'})
        
        stdout = process.communicate()[0]
        stder = process.communicate()[1]
        process.terminate()
        self.create_json_from_traceroute(tmpFile)
        
        print ('STDERR:{}'.format(stder))
